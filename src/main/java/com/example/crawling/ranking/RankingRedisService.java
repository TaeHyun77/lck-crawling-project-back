...
public class RankingRedisService {
    ...
    // Redis 순위 조회
    public Optional<Ranking> getRanking(String stage, String teamName) {
        String key = REDIS_KEY_PREFIX + stage + ":" + teamName;
    ...
    public Ranking getOrUpdateRanking(String stage, String teamName) {
        ...
        Optional<Ranking> dbRanking = rankingRepository.findByTeamNameAndStage(teamName, stage);

        if (dbRanking.isPresent()) {
            Ranking ranking = dbRanking.get();
            log.info("DB에서 순위 데이터 가져옴: " + stage + " " + teamName);
